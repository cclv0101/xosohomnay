    ...
    <table class="table table-hover mb-0">
        ...
        <tbody>
            <?php foreach ($projects as $project) : ?>
                ...
                <tr>
                    <td><span class="text-truncate-3" id="project_title_<?=$project['project_id']?>"><?= $project['project_title'] ?></span></td>
                    <td><img onerror='this.style.display=`none`'id="project_image_<?=$project['project_id']?>" src="<?= $project_image ?>" class="img-fluid"></td>
                    ...
                    <td>
                        <a class="pointer badge green" data-toggle="modal" data-target="#confirmRestore" onclick="restoreProject(<?= $project['project_id'] ?>)">
                            <i class="fa fa-undo"></i> Khôi phục</a>
                        <a class="pointer badge red" data-toggle="modal" data-target="#confirmDelete" onclick="deleteProject(<?= $project['project_id'] ?>)">
                            <i class="fa fa-ban"></i> Xóa vĩnh viễn</a>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
